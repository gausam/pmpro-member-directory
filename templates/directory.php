...
if( $s ) {
	$sql_parts['WHERE'] .= "AND (u.user_login LIKE '%" . esc_sql($s) . "%' OR u.user_email LIKE '%" . esc_sql($s) . "%' OR u.display_name LIKE '%" . esc_sql($s) . "%' OR umf.meta_value LIKE '%" . esc_sql($s) . "%' OR uml.meta_value LIKE '%" . esc_sql($s) . "%') ";
}
